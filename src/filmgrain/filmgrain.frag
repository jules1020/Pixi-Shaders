...
void main(void) {
   ...
   vec3 noise = vec3(pnoise3D(vec3(vTextureCoord * resolution, timer + 0.0)));
   if (colored) {
       noise.g = pnoise3D(vec3(vTextureCoord * resolution, timer + 1.0));
       noise.b = pnoise3D(vec3(vTextureCoord * resolution, timer + 2.0));
   }
